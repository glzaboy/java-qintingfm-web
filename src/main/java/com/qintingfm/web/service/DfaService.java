...
/**
 * Dfa 关键词过滤
 * @link https://blog.csdn.net/xushiyu1996818/article/details/89308561?utm_medium=distribute.pc_relevant.none-task-blog-BlogCommendFromMachineLearnPai2-1.channel_param&depth_1-utm_source=distribute.pc_relevant.none-task-blog-BlogCommendFromMachineLearnPai2-1.channel_param
 * @author guliuzhong
 */
@Service
public class DfaService extends BaseService {
    HashMap<String, Object> dfaMap;
    /**
     * set作为敏感词，创建出对应的dfa的Map，以供检验敏感词
     * @param set 关键词
     */
    public void updateKeyWords(Set<String> set){
        ...
        for(String key:set){
            ...
            for(int i=0;i<key.length();i++){
                ...
                if(nowMap.containsKey("isEnd")&& "1".equals(nowMap.get("isEnd"))){
                    continue;
                }
                if(i!=key.length()-1){
                    nowMap.put("isEnd", "0");
     ...
     * @param useMinMatch 检查类型，如大中华帝国牛逼对应大中华和大中华帝国两个关键字，true为最小检查，会检查出大中华，false位最大，会检查出大中华帝国
     * @return 敏感词
     */
    public Set<String> getSensitiveWord(String string,boolean useMinMatch){
        Set<String> set=new HashSet<>();
        for(int i=0;i<string.length();i++){
            //matchType是针对同一个begin的后面，在同一个begin匹配最长的还是最短的敏感词
            int length=getSensitiveLength(string,i,useMinMatch);
            if(length>0){
     ...
     * @param string
     * @param beginIndex
     * @param useMinMatch  true：最小匹配规则，false：最大匹配规则
     * @return 敏感词字符的长度不存在返回0
     */
    public int getSensitiveLength(String string,int beginIndex,boolean useMinMatch){
        //当前匹配的长度
        int nowLength=0;
